...
{
    if (G_Parallel(ray.dir, line.dir))
        return 0;
...
int FindRegion(Box rect, Vector p)
{
    ...
    if (p.y < rect.top && !EQ(p.y, rect.top))
        code |= G_REGION_TOP;
    if (p.y > rect.bottom && !EQ(p.y, rect.bottom))
        code |= G_REGION_BOTTOM;
    if (p.x > rect.right && !EQ(p.x, rect.right))
        code |= G_REGION_RIGHT;
    if (p.x < rect.left && !EQ(p.x, rect.left))
        code |= G_REGION_LEFT;
